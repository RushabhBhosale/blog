...
const CANONICAL = `${SITE}/mini-sparks`;
...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    title: "Mini Sparks — Short takes and quick reads",
    description: "Bite-sized movie reviews and travel notes in 100–200 words.",
    images: [{ url: `${SITE}/og/minisparks-og.jpg`, width: 1200, height: 630 }],
  },
  twitter: {
    card: "summary_large_image",
    title: "Mini Sparks — Short takes and quick reads",
    description: "Bite-sized movie reviews and travel notes in 100–200 words.",
    images: [`${SITE}/og/minisparks-og.jpg`],
  },
...
export default async function MiniSparksPage() {
  ...
  const itemListElements =
    items?.map((it: any, i: number) => ({
      "@type": "ListItem",
      position: i + 1,
      url: `${SITE}/mini-sparks/${encodeURIComponent(it.slug)}`,
      name: it.title,
    })) ?? [];
  ...
  const webPageLd = {
    ...
    url: CANONICAL,
    name: "Mini Sparks — Short takes and quick reads",
    inLanguage: "en",
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-10">
      <Script
        id="jsonld-minisparks"
        type="application/ld+json"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify([breadcrumbLd, webPageLd, collectionLd]),
        }}
      />

      <header className="mb-6 flex items-end justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Mini Sparks</h1>
      ...
      {items.length === 0 ? (
        ...
        <section className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((it: any) => {
            const href = `/mini-sparks/${encodeURIComponent(it.slug)}`;
            const ratingPercent =
              ...
              >
                ...
                <div className="p-4 flex-1 flex flex-col">
                  ...
                  <p
                    ...
                    // content is trusted HTML from your DB; you already sanitize on detail
